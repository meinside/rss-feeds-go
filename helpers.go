...
var (
	...
	reItalicMarkdown        = regexp.MustCompile(`\*([^*]+)\*`)
	reCodeMarkdown          = regexp.MustCompile("`([^`]+)`")
...
func decorateHTML(body string) string {
	...
	// Otherwise, escape and convert markdown in `body` which was successfully generated by Gemini:
	if !isError(body) {
		...
		body = reItalicMarkdown.ReplaceAllString(body, `<i>$1</i>`)
		body = reCodeMarkdown.ReplaceAllString(body, `<code>$1</code>`)
		body = reLinkMarkdown.ReplaceAllString(body, `<a href="$2">$1</a>`)
	} else {
		// for error bodies, only apply bold conversion
		body = reBoldMarkdown.ReplaceAllString(body, `<b>$1</b>`)
	}
